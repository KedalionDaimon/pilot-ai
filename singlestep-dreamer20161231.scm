; This is a variation of the Singlestep Piloting System.
...
; just as it is handy, though not used here:
...
(define x-knowledge (with-input-from-file "pilots3.txt" read))
...
(define (pseudo-run) ; no reply printing, no reading of x-human
  (begin
    ...
    (set! x-machine (free-flight x-history x-knowledge))))

(define (eexxiitt)
(begin (newline)
(with-output-to-file "pilots3.txt" (lambda () (display x-knowledge)))
(exit)))
...
; 1. you could do the above line several times;
; 2. you could set the dreams to be longer.
